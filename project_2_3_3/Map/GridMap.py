...
class GridMap(Box):
    ...
    def InitNodeMapByRandom(self, boxNumber=3):
        ...
        randomBox = RandomBoxGenerator(boxNumber, 0, self.MapSize - 1)
    ...
    def InitTlt(self, pipeShape: TopoDS_Shape):
        tlt = TessellatorShape(pipeShape, self.Display)
        tlt.mesh.RandomSampling(1000)
        self.PntList = tlt.mesh.GetPoints()

    def CheckCollapse(self, obstacle: TopoDS_Shape) -> bool:
        classifier = BRepClass3d_SolidClassifier(obstacle)

        pntList = self.PntList
        for vertex in pntList:
            classifier.Perform(vertex, 1e-6)
