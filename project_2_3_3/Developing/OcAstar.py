...
class OcAstar:
    ...
    def DisplayPathByPnt(self, _transparency: float = 0, _color='blue', diameter: float = 3, gap: float = 0.0) -> None:
        for pnt in self.PathPoints:
            self.Display.DisplayShape(pnt.CenterPoint)
            print(pnt.CenterPoint.XYZ().Coord())
    ...
    def DisplayPathByPipe(self, _transparency: float = 0, _color='blue', diameter: float = 3, gap: float = 0.0) -> None:
        ...
        for i in range(length - 2):
            directionEdgeBRep = BRepBuilderAPI_MakeEdge(
                pointList[i], pointList[i + 1])
            directionEdgeBRep.IsDone()
            directionEdge = directionEdgeBRep.Shape()
            directionWireBRep = BRepBuilderAPI_MakeWire(directionEdge)
            
            directionWireBRep.IsDone()
            directionWire = directionWireBRep.Shape()


            directionCircle: gp_Circ = gp_Circ(gp_Ax2(self.PathPoints[i + 1].CenterPoint, gp_Dir(
                pointList[i + 1].XYZ().Subtracted(pointList[i].XYZ()))), diameter)
            
            directionCircleEdgeBRep= BRepBuilderAPI_MakeEdge(
                directionCircle)
            directionCircleEdge = directionCircleEdgeBRep.Shape()
            
            directionCircleWireBRep = BRepBuilderAPI_MakeWire(
                directionCircleEdge)
            directionCircleWire = directionCircleWireBRep.Wire()
            pipeShapeBRep =  BRepOffsetAPI_MakePipe(
                directionWire, directionCircleWire)
            pipeShape: TopoDS_Shape = pipeShapeBRep.Shape()
