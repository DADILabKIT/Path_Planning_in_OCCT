...
class Agent:
    def __init__(self, startNode: Node = None, endNode: Node = None, nodeMap: GridMap = None, agentName: str = ' ', display: Viewer3d = None) -> None:
        ...
        self.ClosedList: list[list[list[bool]]] = [[[False for _ in range(self.N)] for _ in range(
            self.N)] for _ in range(self.N)] if isinstance(self.N, int) else None
        # display
        self.Display: Viewer3d = display
        self.AgentName: str = agentName
        self.PipeShape: TopoDS_Shape = None
    ...
    def InitDistance(self) -> None:
        ...
        for i in range(length - 1):
            result += self.PathPoints[i].CenterPoint.Distance(
                self.PathPoints[i + 1].CenterPoint)
    ...
    def InitDegree(self) -> None:
        ...
        for i in range(length - 2):
            v1 = gp_Vec(self.PathPoints[i].CenterPoint.XYZ(
            ) - self.PathPoints[i + 1].CenterPoint.XYZ())
            v2 = gp_Vec(self.PathPoints[i + 1].CenterPoint.XYZ() -
                        self.PathPoints[i + 2].CenterPoint.XYZ())
            v2.Reverse()
    ...
    def DisplayPathByPipe(self, _transparency: float = 0, _color='blue', diameter: float = 3, gap: float = 0.0) -> None:
        ...
        for i in range(length - 1):
            directionEdge: TopoDS_Edge = BRepBuilderAPI_MakeEdge(
                pointList[i], pointList[i + 1]).Edge()
    ...
    def DisplayPathBySplinePipe(self, _transparency: float = 0, _color='blue', diameter: float = 3, gap: float = 0.0, startGp: gp_Pnt = None, endGp: gp_Pnt = None, d = False) -> None:
        pointList: list[gp_Pnt] = []
        if (endGp):
    ...
    def DisplayUnifromedSpline(self, _diameter: float = 5.0, _color: str = "red", idx: list[int] = None, d = False):
        ...
        if (N == 2 or N == 1 or N == 3):
            ...
            K = int(N / 2)
        mults = TColStd_Array1OfInteger(1, K)
        s1 = 0
    
        for i in range(K):
            if (i == 0 or i == K - 1):
                mults.SetValue(i + 1, 4)    
                s1 += 4
                continue
            mults.SetValue(i + 1, 0)
            s1 += 0
        if (len(self.PathPoints) != s1 - 1 - degree):
            p1 = len(self.PathPoints) - s1 + 1 + degree
            j = 2
            while (p1 and p1 != -1):
                mults.SetValue(j, mults.Value(j) + 1)
                j = (j + 1) % (K)
                j = max(j ,2)
                p1 -= 1

        knots = TColStd_Array1OfReal(1, K)
        for i in range(K):
            knots.SetValue(i + 1, (i) * 1.0/(K))
        
        poles = TColgp_Array1OfPnt(1, N)
        weights = TColStd_Array1OfReal(1, N)

        for i in range(N):
            poles.SetValue(i + 1, self.PathPoints[i].CenterPoint)
            weights.SetValue(i + 1, 1)
        # idx를 통해 가중치 설정
        if (idx):
            for i in idx:
                weights.SetValue(i + 1, 10)
        ...
        diameter = _diameter
        direction_vec = gp_Vec(self.PathPoints[1].CenterPoint.XYZ()) - gp_Vec(self.PathPoints[0].CenterPoint.XYZ())
        direction_dir = gp_Dir(direction_vec)
        circle = Geom_Circle(gp_Ax2(self.PathPoints[0].CenterPoint, direction_dir), diameter / 2)
        section = BRepBuilderAPI_MakeEdge(circle).Edge()
        spline = BRepOffsetAPI_MakePipe(wire, section).Shape()

        self.PipeShape = spline
        
        if (d):
            self.Display.DisplayShape(spline, color= _color)
        print("weight : ")
        for i in range(N):
            w1 = weights.Value(i + 1)
            print(w1, end = ' ')
        print()
        pass
