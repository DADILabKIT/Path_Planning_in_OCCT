...
class GridMap(Box):
    ...
    def InitNodeMapByRandom(self, boxNumber = 3):
        ...
        for i in range(0, len(randomBox.RandBox) - 1, 2):
            s1, t1 = min(randomBox.RandBox[i][0], randomBox.RandBox[i + 1][0]), max(randomBox.RandBox[i][0], randomBox.RandBox[i + 1][0]) + 1
            for x in range(s1, t1):
                s2, t2 = min(randomBox.RandBox[i][1], randomBox.RandBox[i + 1][1]), max(randomBox.RandBox[i][1], randomBox.RandBox[i + 1][1]) + 1
