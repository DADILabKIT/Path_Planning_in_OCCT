...
class TessellatorCompound:
    def __init__(self, compound: TopoDS_Compound, display: Viewer3d) -> None:
        self.cnt = 0
        self.Display = display
        self.mesh = Mesh(display)
        self.Compound: TopoDS_Compound = compound
        self.InitCompoundMesh()
        
    def InitCompoundMesh(self):
        shapeIter: TopoDS_Iterator = TopoDS_Iterator(self.Compound)
        while (shapeIter.More()):
            shape = shapeIter.Value()
            self.InitShapeBndBox(shape)
            shapeIter.Next()
            
    
    def InitShapeBndBox(self, shape : TopoDS_Shape):
        ...
        self.GetVertexPnt(shape)

    def GetVertexPnt(self, shape):
        explorer = TopExp_Explorer()

        explorer.Init(shape, TopAbs_FACE)

        if (not self.BRepMesh.IsDone()):
            return
